...
SMTP_PASS = "D45192091425Ea@"   # ⚠️ Move to env vars in production

ADMIN_EMAIL = "support@kredinou.com"   # Change to your real admin email


def send_admin_notification(loan):
    """
    Sends an email to the admin when a new loan application is submitted.
    """
    try:
...
def send_user_confirmation(loan):
    ...
    try:
        ...
        with smtplib.SMTP_SSL(SMTP_SERVER, SMTP_PORT_SSL, context=context) as server:
            server.login(SMTP_USER, SMTP_PASS)
            server.sendmail(SMTP_USER, loan['user']['email'], msg.as_string())

        print(f"✅ Confirmation email sent to {loan['user']['email']}")
